...
### Phase R. 테이블 레이어 검증 고도화 (2026-02-24)

#### ROOM-UI-005 테이블 레이어 QA/자산 정합성
- `ROOM-UI-005C`: `scripts/qa/room-table-layer-assets.ts` HTTP 검증 경로를 고정 실행 가능한 명령/문서로 연결하고 skip 없는 실행 모드 추가
- `ROOM-UI-005D`: 레거시 `table-top.png` 자산 정책 확정(제거 또는 fallback 명시) 및 코드/문서 정리
- `ROOM-UI-005E`: 공 가시 경계와 물리 경계(테이블 내 이동 가능 범위) 일치성 검증 스크립트 추가
- `ROOM-UI-005F`: 브라우저 기반 레이어 렌더 회귀(프레임/천/공 순서) 검증 자동화 추가
- `ROOM-UI-005G`: 반응형 리사이즈(모바일/데스크톱)에서 2:1 비율 유지 및 좌표 왜곡 회귀 테스트 추가
