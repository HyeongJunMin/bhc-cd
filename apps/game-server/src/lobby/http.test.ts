...
test('턴 타이머: 샷 제출 시 turnDeadlineMs가 null이 되고, 턴 전환 시 새로 설정된다', async () => {
  const { state } = createLobbyHttpServer();
  const created = createRoom(state, { title: 'timer-test' });
  assert.equal(created.ok, true);
  if (!created.ok) return;
  joinRoom(state, created.room.roomId, { memberId: 'u1', displayName: 'host' });
  joinRoom(state, created.room.roomId, { memberId: 'u2', displayName: 'guest' });
  ...
  // 게임 시작 후 turnDeadlineMs가 설정되어 있어야 한다
  assert.ok(created.room.turnDeadlineMs !== null, '게임 시작 후 turnDeadlineMs가 설정되어야 한다');
  assert.ok(Number.isFinite(created.room.turnDeadlineMs), 'turnDeadlineMs는 유한한 숫자여야 한다');
  ...
  // 샷 제출 후 turnDeadlineMs가 null이어야 한다
  assert.equal(created.room.turnDeadlineMs, null, '샷 제출 후 turnDeadlineMs가 null이어야 한다');

  // 공 정지 → 턴 전환 대기
  forceBallsSettled(created.room);
  await waitUntil(() => created.room.turnDeadlineMs !== null);

  // 턴 전환 후 turnDeadlineMs가 새로 설정되어야 한다
  assert.ok(created.room.turnDeadlineMs !== null, '턴 전환 후 turnDeadlineMs가 다시 설정되어야 한다');
  assert.ok(Number.isFinite(created.room.turnDeadlineMs), '새 turnDeadlineMs는 유한한 숫자여야 한다');
});
