...
async function checkLocalAssets(repoRoot: string): Promise<void> {
  ...
  console.log(`ROOM-UI-005B local pass: cloth=${cloth.width}x${cloth.height}, frame=${frame.width}x${frame.height}`);
}
...
async function checkRenderSource(repoRoot: string): Promise<void> {
  ...
  console.log('ROOM-UI-005B source pass: frame->cloth render/load order verified');
}
...
async function checkHttpAssets(baseUrl: string): Promise<void> {
  ...
  console.log(`ROOM-UI-005B http pass: baseUrl=${baseUrl}`);
}
...
async function run(): Promise<void> {
  ...
  if (!baseUrl) {
    console.log('ROOM-UI-005B http skip: QA_BASE_URL 미지정');
    return;
  }
